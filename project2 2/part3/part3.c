...
void pid_info (int pid) {
    ...
    if (fp != NULL) {
          while ((nread = getline(&line, &len, fp)) != -1) {
              ...
              if (strcmp(command_arry[0], "Name:\0") == 0) {
                  printf("%s\n", line);
                }
...
void scheduler (pid_t* pid_ary, int size, sigset_t set, int sig) {
    int status;
    int status1;
    int ret_val;
    int exit_val;
    ...
    sigprocmask(SIG_BLOCK, &set, NULL);
        ...
        while(1) {
          ...
          if (sigwait(&set, &sig) == 0){
            ...
            state = waitpid(pid_ary[(j)%size], &status, WUNTRACED | WCONTINUED | WNOHANG /*| WEXITED*/);
